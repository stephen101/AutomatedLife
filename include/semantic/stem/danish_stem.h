...
namespace stemming
	...
	public:
		...
		void operator()(std::basic_string<xchar>& text)
			...
			find_r1(text, _XTEXT("aeiouyæåøAEIOUYÆÅØ"));
			if (m_r1 == text.length() )
				{
				return;
				}
	...
	private:
		...
		void step_1(std::basic_string<xchar>& text)
			...
			/**Define a valid s-ending as one of 
				...
				{
				if (string_util::is_one_of(text[text.length()-2], _XTEXT("abcdfghjklmnoprtvyzåABCDFGHJKLMNOPRTVYZÅ")) )
					{
		...
		void step_4(std::basic_string<xchar>& text)
			...
			if (m_r1 <= text.length()-1 &&
				...
				{
				if (!string_util::is_one_of(text[text.length()-2], _XTEXT("aeiouyæåøAEIOUYÆÅØ")) )
					{
					text.erase(text.end()-1, text.end() );
					update_r_sections(text);
					}
