...
namespace stemming
	...
	public:
		...
		void operator()(std::basic_string<xchar>& text)
			...
			italian_acutes_to_graves(text);
			hash_italian_ui(text, _XTEXT("aeiouàèìòùAEIOUÀÈÌÒÙ"));

			find_r1(text, _XTEXT("aeiouàèìòùAEIOUÀÈÌÒÙ"));
			find_r2(text, _XTEXT("aeiouàèìòùAEIOUÀÈÌÒÙ"));
			find_spanish_rv(text, _XTEXT("aeiouàèìòùAEIOUÀÈÌÒÙ"));
	...
	private:
		...
		void step_3a(std::basic_string<xchar>& text)
			...
			if (m_rv <= text.length()-1 &&
				string_util::is_one_of(text[text.length()-1], _XTEXT("aeioàèìòAEIOÀÈÌÒ")) )
				{
