	...
	private:
		...
		void step_1(std::basic_string<xchar>& text)
			...
			///Define a valid en-ending as a non-vowel, and not gem.
				...
				if (m_r1 <= text.length()-3 &&
					...
					!(is_either(text[text.length()-6], LOWER_G, UPPER_G) &&
					is_either(text[text.length()-5], LOWER_E, UPPER_E) &&
					is_either(text[text.length()-4], LOWER_M, UPPER_M) ) ) )
					{
				...
				if (m_r1 <= text.length()-2 &&
					...
					(text.length() < 5 ||
					!(is_either(text[text.length()-5], LOWER_G, UPPER_G) &&
					is_either(text[text.length()-4], LOWER_E, UPPER_E) &&
					is_either(text[text.length()-3], LOWER_M, UPPER_M) ) ) )
					{
			...
			else if (is_suffix(text,/*se*/LOWER_S, UPPER_S, LOWER_E, UPPER_E) &&
				!string_util::is_one_of(text[text.length()-3], _XTEXT("aeèiouyjAEÈIOUYJ")))
				{
				if (m_r1 <= text.length()-2)
					{
					text.erase(text.end()-2, text.end() );
			...
			else if (is_suffix(text, LOWER_S, UPPER_S) &&
				!string_util::is_one_of(text[text.length()-2], _XTEXT("aeèiouyjAEÈIOUYJ")))
				{
		...
		void step_2(std::basic_string<xchar>& text)
			...
			if (is_suffix(text, LOWER_E, UPPER_E) )
				...
				if (m_r1 <= text.length()-1 &&
					...
					if (string_util::is_one_of(text[text.length()-2], _XTEXT("iyIY")) &&
						string_util::is_one_of(text[text.length()-3], DUTCH_VOWELS) )
						{
						return;
						}
		...
		void step_3a(std::basic_string<xchar>& text)
			...
			if (is_suffix(text,/*heid*/LOWER_H, UPPER_H, LOWER_E, UPPER_E, LOWER_I, UPPER_I, LOWER_D, UPPER_D) )
				...
				if (text.length() >= 5 &&
					m_r2 <= text.length()-4 &&
					is_neither(text[text.length()-5], LOWER_C, UPPER_C) )
					{
					...
					if (m_r1 <= text.length()-2 &&
						...
						if ((!string_util::is_one_of(text[text.length()-3], DUTCH_VOWELS) ||
							(string_util::is_one_of(text[text.length()-3], _XTEXT("iyYI")) &&
							string_util::is_one_of(text[text.length()-4], DUTCH_VOWELS))) &&
							(text.length() < 5 ||
							!(is_either(text[text.length()-5], LOWER_G, UPPER_G) &&
							is_either(text[text.length()-4], LOWER_E, UPPER_E) &&
		...
		void step_3b(std::basic_string<xchar>& text)
			...
			if (delete_if_is_in_r2(text,/*end*/LOWER_E, UPPER_E, LOWER_N, UPPER_N, LOWER_D, UPPER_D) ||
				...
				if (text.length() > 3)
					{
					if (is_neither(text[text.length()-3], LOWER_E , UPPER_E) &&
			...
			else if (!(text.length() >= 3 && is_either(text[text.length()-3], LOWER_E, UPPER_E) ) &&
					delete_if_is_in_r2(text,/*ig*/LOWER_I, UPPER_I, LOWER_G, UPPER_G) )
				{
				update_r_sections(text);
				return;
		...
		void step_4(std::basic_string<xchar>& text)
			...
			if (text.length() >= 4 &&
				...
				string_util::is_one_of(text[text.length()-2], _XTEXT("aeouAEOU")) &&
